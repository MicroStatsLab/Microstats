...
growthRates takes in two dataframes (`d` and `wells`) as well as logical and string values indicating which output you are looking for and any additional information. The first of which, `plotRaw` indicates whether you would like to have your raw data plotted.By default, plotRaw is set to true, but can be changed to false by indicating `plotRaw = FALSE`. If you would like it to remain true, you can ommit setting this parameter when calling the function. The second parameter is `strainName` which takes in a string value of the strain's name.
