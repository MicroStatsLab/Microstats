...
The <i>Microstats</i> package allows users to analyze microbial growth data into quantifiable values and images, combine two different results files, and calculate the variance among photographs of the same type.


### <i>Microstats</i> Function Overview
*`growthRates`: used to calculate, plot, and test microbial data.
*`combineImageJ`: used to combine two .csv into one .csv file.
*`aggregateData`: used to average photographs of the same type.
...
### Preparing Data: For `growthRates`
...
### Preparing Data: For `combineImageJ`
`combineImageJ` requires two dataframes. The first containing the summary results of the total area of the epithelial barrier thickness assay images, this dataframe is referred to as `area_total`. The second dataframe should contain the measurements of the individual red, green, and blue colouring from the thickness assay images. This dataframe is referred to as `area_RGB`. For both dataframes, the headings of the columns should remain the same as what ImageJ outputs.
...
```r
growthRates(myODData, myWellData, plotRaw = TRUE, strainName = "Strain's name")
```
