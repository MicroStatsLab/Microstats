...
#read in OD values (the data)
d <- read.csv("GrowthRates_forLGR.csv") #this file name should be free to be anything
names(d) <- c("t", paste("C", seq(length=ncol(d)-1), sep="")) # this makes sure the column names are as expected for the script to work- namely that there is a "t" column
d <- d[complete.cases(d$t),]

#read in wells information (the meta data)
wells <- read.csv("GrowthRates_forLGR_wells.csv") #this file name should be free to be anything
...
# this doesn't necessarily need to get coded, and there won't always be an environment column
